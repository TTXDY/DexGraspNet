"""
Generate grasps for test objects
"""
...
parser.add_argument('--name', default='test_grasps', type=str)
parser.add_argument('--n_contact', default=4, type=int)
parser.add_argument('--batch_size', default=32, type=int)  # Reduced for testing
parser.add_argument('--n_iter', default=3000, type=int)  # Reduced for testing
...
device = torch.device('cpu')  # Force CPU mode to avoid pytorch3d issues
print('Running on:', device)
print('WARNING: Running in CPU-only mode. This will be slower but avoids CUDA compatibility issues.')
print('Objects:', args.object_code_list)
print('Batch size per object:', args.batch_size)
...
for step in tqdm(range(1, args.n_iter + 1), desc='Optimizing grasps'):
...
for i in range(len(args.object_code_list)):
    ...
    output_file = os.path.join(result_path, args.object_code_list[i] + '.npy')
    np.save(output_file, data_list, allow_pickle=True)
    print(f'Saved: {output_file}')

print(f'\nDone! Results saved to: {result_path}')
print(f'View logs with: tensorboard --logdir=../data/experiments/{args.name}/logs')
